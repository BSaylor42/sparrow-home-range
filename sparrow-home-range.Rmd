...
```
The graph shows the weighted autocorrelated kernel density estimate (wAKDE) which estimated the animal's space use while accounting for location error and temporal autocorrelation. Nugget's wAKDE is very flat mostly indicating mostly regular fixes on GPS. The sudden rise at the right of the graph indicated a few long gaps occurred where the GPS lost signal. This is probably due to Nugget leaving Emerald Hill in the evening to go sleep in another location. 
...
```{r ggplot wADKE, echo=TRUE, fig.height=8, fig.width=10, message=FALSE, warning=FALSE, fig.align='center'}
# ggplot for akde
...
```{r comparison, echo=TRUE, fig.height=8, fig.width=10, message=FALSE, warning=FALSE, fig.align='center'}
